...
export function parseWidgetData(data, widgetType) {
  ...
  if (!content) {
    console.warn(`[widget-parser] No content to parse for widget: ${widgetType}`);
    return {};
  }
  ...
  try {
    ...
    console.warn(`[widget-parser] Widget token not found or no data for: ${widgetType}`);
    return {};
  } catch (e) {
    console.error(`[widget-parser] Parse data failed for ${widgetType}:`, e);
    // 出错时也尝试直接提取
...
function extractJsonDirectly(content, widgetType) {
  ...
  try {
    ...
    console.warn(`[widget-parser] Direct JSON parse failed:`, e);
  }
